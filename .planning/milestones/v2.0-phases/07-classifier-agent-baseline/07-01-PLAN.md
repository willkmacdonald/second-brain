...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/tests/test_transcription.py"
      provides: "Unit tests for TranscriptionTools.transcribe_audio with mocked success/failure"
      contains: "transcribe_audio"
  key_links:
    ...
    - from: "backend/src/second_brain/tools/transcription.py"
      ...
      via: "audio.transcriptions.create() with self._deployment_name (4-param constructor: openai_client, blob_manager, credential, deployment_name)"
...
1. Create `TranscriptionTools` class with `__init__(self, openai_client: AsyncAzureOpenAI, blob_manager: BlobStorageManager, credential: AsyncDefaultAzureCredential, deployment_name: str)`:
   - Store `deployment_name` as `self._deployment_name` for use in the transcription API call
   - Store `credential` as `self._credential` for blob download auth
2. Add `transcribe_audio` tool with `@tool(approval_mode="never_require")`:
   ...
   - Calls `self._openai.audio.transcriptions.create(model=self._deployment_name, file=("recording.m4a", audio_bytes, "audio/m4a"))` where `_deployment_name` is from the constructor
   ...
   - The `_download_blob` helper method uses `self._credential` from the constructor rather than creating a new one
...
**Create `test_transcription.py`** -- new unit test file for TranscriptionTools (AGNT-05 behavioral coverage):

1. Create `_make_transcription_tools()` helper:
   - Mock `AsyncAzureOpenAI` with `audio.transcriptions.create` as an `AsyncMock`
   - Mock `BlobStorageManager`
   - Mock `AsyncDefaultAzureCredential`
   - Pass `deployment_name="gpt-4o-transcribe"` (matches 4-param constructor)
   - Return the tools instance and the mocks

2. `test_transcribe_audio_success`:
   - Mock `BlobClient.from_blob_url` to return a mock blob client whose `download_blob().readall()` returns `b"fake-audio-bytes"`
   - Mock `openai_client.audio.transcriptions.create` to return a mock with `.text = "Pick up prescription at Walgreens"`
   - Call `await tools.transcribe_audio(blob_url="https://storage.blob.core.windows.net/voice/recording.m4a")`
   - Assert result == `"Pick up prescription at Walgreens"` (plain transcript text string per CONTEXT.md)
   - Assert `openai_client.audio.transcriptions.create` was called with `model="gpt-4o-transcribe"`

3. `test_transcribe_audio_blob_download_failure`:
   - Mock `BlobClient.from_blob_url` to raise `Exception("Blob not found")`
   - Call `await tools.transcribe_audio(blob_url="https://storage.blob.core.windows.net/voice/missing.m4a")`
   - Assert result is an error string containing `"Transcription error"` (per CONTEXT.md: no exceptions raised)

4. `test_transcribe_audio_openai_api_failure`:
   - Mock blob download succeeds but `openai_client.audio.transcriptions.create` raises `Exception("API rate limit")`
   - Call `await tools.transcribe_audio(blob_url=...)`
   - Assert result is an error string containing `"Transcription error"` (per CONTEXT.md: no exceptions raised)
  </action>
  <verify>
...
Run `cd /Users/willmacdonald/Documents/Code/claude/second-brain/backend && python3 -m pytest tests/test_classification.py tests/test_transcription.py -v` -- all tests pass.

Run `cd /Users/willmacdonald/Documents/Code/claude/second-brain/backend && ruff check src/second_brain/agents/ src/second_brain/tools/ tests/test_classification.py tests/test_transcription.py` -- no errors.
  </verify>
  <done>
AuditAgentMiddleware and ToolTimingMiddleware implement AgentMiddleware/FunctionMiddleware with correct process() signatures. Classifier module has ensure_classifier_agent + CLASSIFIER_INSTRUCTIONS. All unit tests pass with file_capture dict returns and transcribe_audio success/failure scenarios. mark_as_junk and request_misunderstood tests are removed.
  </done>
</task>
...
2. `python3 -m pytest tests/test_classification.py tests/test_transcription.py -v` -- all tests pass
...
- All unit tests pass with updated assertions for dict returns and transcribe_audio success/failure coverage
