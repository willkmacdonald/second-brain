...
must_haves:
  truths:
    ...
    - "main.py lifespan creates AzureAIAgentClient AND makes a probe call (agents.list with limit=1) to validate auth before storing on app.state — raises on failure (genuine fail fast)"
...
1. After Key Vault fetch, after Cosmos DB init, add a new "Foundry Agent Service" initialization block:
   ...
   try:
       ...
       # AzureAIAgentClient is a lazy client -- construction alone does NOT
       # make a network call, so wrong credentials would pass silently.
       # Force an auth round-trip to genuinely validate connectivity + RBAC.
       await foundry_client.agents.list(limit=1)
   ...
   **Why the probe call:** `AzureAIAgentClient` constructor is lazy (no network I/O). Without the
   `await foundry_client.agents.list(limit=1)` probe, the try/except would never catch RBAC or
   credential errors — the container would report `foundry: connected` even with wrong credentials
   until Phase 7 makes the first real API call. The `list(limit=1)` call is the lightest-weight
   operation that exercises the full auth path (credential -> token -> Foundry API).

   **Implementation note:** If `agents.list()` does not accept a `limit` keyword, try
   `agents.list_agents(limit=1)` or the equivalent method name from the SDK. The executor should
   check the actual `AzureAIAgentClient.agents` API surface during implementation and use whatever
   list/get method is available. The key requirement is: at least one `await` call that hits the
   Foundry API before storing the client on `app.state`.
...
Note: The health endpoint uses `getattr` with defaults for safety. The Foundry status is "connected" if the client was created successfully in lifespan AND the probe call passed (which validates credential + RBAC + endpoint). The fail-fast in lifespan (including the probe) guarantees that if the app is running, Foundry auth has been validated.
