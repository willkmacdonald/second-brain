...
must_haves:
  truths:
    ...
    - "No audio files remain in Azure Blob Storage voice-recordings container after a successful voice capture"
    - "Orchestrator routes transcribed voice text through the same Classifier pipeline as text captures (same classification quality)"
  artifacts:
...
The SSE generator manages the run lifecycle MANUALLY (do NOT delegate to `_stream_sse()` -- that helper wraps the stream in its own RunStartedEvent + RunFinishedEvent, which would emit duplicate lifecycle events and break the mobile SSE parser). Follow the same manual pattern used by `follow_up_misunderstood` in main.py (lines 532-754):

1. Create encoder: `encoder = EventEncoder()`
2. Yield `encoder.encode(RunStartedEvent(thread_id=thread_id, run_id=run_id))`
3. **Perception step** (synthetic -- not via agent framework):
...
4. **Classification pipeline** -- Feed transcribed text through existing workflow:
   ...
   - Iterate the stream items DIRECTLY (same pattern as follow_up_misunderstood generator):
     ```python
     ...
     if message_id_state.get("current"):
         yield encoder.encode(TextMessageEndEvent(message_id=message_id_state["current"]))
     ```
   - The workflow stream emits Orchestrator/Classifier step events and CustomEvents (CLASSIFIED, HITL_REQUIRED, MISUNDERSTOOD) which pass through directly
5. Yield `encoder.encode(RunFinishedEvent(thread_id=thread_id, run_id=run_id))`
