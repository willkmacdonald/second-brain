...
<task type="auto">
  ...
  <action>
    ...
    **main.py updates:**
    ...
    - Add middleware AFTER `add_agent_framework_fastapi_endpoint()` call so it wraps the AG-UI endpoint (per research Pitfall 3 — `app.add_middleware()` always adds to outermost layer, so order of `add_middleware` calls does not matter, but adding AFTER registration is the documented safe pattern)
    - Pass `api_key=app.state.api_key` to the middleware constructor (API key was fetched from Azure Key Vault during lifespan startup in Plan 01-01, NOT from pydantic-settings)
