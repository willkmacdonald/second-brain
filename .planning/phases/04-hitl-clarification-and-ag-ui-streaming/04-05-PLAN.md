...
<task type="auto">
  <name>Task 2: Update capture screen and conversation screen for real clarification + DB filing</name>
  <files>
    mobile/app/capture/text.tsx
    mobile/app/conversation/[threadId].tsx
  </files>
...
const question = (item as any)?.clarificationText
  || "Which bucket does this belong to?";
```

Note: The `(item as any)` cast is needed because `InboxItemData` from `InboxItem.tsx` may not have the `clarificationText` field typed yet. Alternatively, update the `InboxItemData` interface to include `clarificationText?: string`. Check the InboxItemData definition and add the field there if possible.
...
Extract top 2 buckets from classificationMeta.allScores:
```typescript
// Derive top 2 buckets from classification scores
const topBuckets: string[] = [];
if (item?.classificationMeta?.allScores) {
  const scores = item.classificationMeta.allScores as Record<string, number>;
...
**d) Update InboxItemData type if needed:**

Check `mobile/components/InboxItem.tsx` for the `InboxItemData` interface. If it doesn't have `clarificationText`, add it:
```typescript
clarificationText?: string;
```

This avoids the `as any` cast in the conversation screen.
  </action>
  <verify>
1. `cd /Users/willmacdonald/Documents/Code/claude/second-brain/mobile && npx tsc --noEmit` — TypeScript compiles without errors
2. Grep text.tsx for "inboxItemId" — present in handleBucketSelect sendClarification call and state
3. Grep text.tsx for "bucketButtonPrimary" — primary/secondary styling exists
4. Grep conversation/[threadId].tsx for "inboxItemId" — present in sendClarification call with item?.id
5. Grep conversation/[threadId].tsx for "clarificationText" — used for question display
6. Grep conversation/[threadId].tsx for "bucketButtonPrimary" — primary/secondary styling exists
7. No hardcoded "Which bucket does this belong to?" as sole question source (should be fallback only)
  </verify>
  <done>
