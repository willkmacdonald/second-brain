...
<task type="auto">
  ...
  <done>
    New POST /api/capture/follow-up/voice endpoint accepts multipart audio + inbox_item_id, transcribes via gpt-4o-transcribe, then streams follow-up reclassification with reconciliation (which handles both CLASSIFIED and LOW_CONFIDENCE per 09-04). Blob cleaned up after stream completes. Existing text follow-up endpoint unchanged.
  </done>
</task>
  ...
  <action>
    ...
    **text.tsx (text capture screen -- does NOT have audio recording):**
    ...
    6. Create `handleVoiceFollowUpSubmit` that:
       ...
       - Calls `sendFollowUpVoice({ audioUri, inboxItemId: misunderstoodInboxItemId, followUpRound, apiKey: API_KEY!, callbacks })` with the same callbacks as `handleFollowUpSubmit` (onMisunderstood, onComplete, onError, onUnresolved, onLowConfidence)
...
7. text.tsx has all audio recording infrastructure (recorder, state, permissions, Animated import)
