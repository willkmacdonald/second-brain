...
must_haves:
  ...
  key_links:
    ...
    - from: "backend/src/second_brain/api/capture.py"
      to: "CosmosDB Inbox container"
      via: "stream_with_thread_id_persistence upserts foundryThreadId after MISUNDERSTOOD"
      pattern: "foundryThreadId"
...
<task type="auto">
  ...
  <action>
    ...
    **5. Write `foundryThreadId` to Cosmos after initial capture stream** (`api/capture.py`):
    The initial `POST /api/capture` endpoint streams classification via `stream_text_capture` (and `stream_voice_capture`). When the outcome is MISUNDERSTOOD, the SSE event contains `foundryConversationId` (added in step 2 above), but nobody persists it to Cosmos yet. Without this write, the follow-up endpoint will always return 400 ("No thread ID for follow-up").

    Add a wrapper generator (e.g., `stream_with_thread_id_persistence`) in `capture.py` that:
    a) Wraps the inner generator (`stream_text_capture` or `stream_voice_capture`)
    b) Yields all SSE events through to the client as-is
    c) Intercepts MISUNDERSTOOD events by parsing the SSE `data:` line as JSON
    d) When a MISUNDERSTOOD event is detected, extracts `inboxItemId` and `foundryConversationId` from the payload
    e) After the inner generator completes (i.e., after all events are yielded), if a MISUNDERSTOOD event was detected, upserts `foundryThreadId` to the inbox document in Cosmos:
    ```python
    ...
    ```
    f) The wrapper needs access to `request` (for `request.app.state.cosmos_manager`), so define it as an inner function within the endpoint handler or pass cosmos_manager as a parameter.

    Apply this wrapper to both `capture_text()` and `capture_voice()` endpoint handlers. The wrapper goes around the `StreamingResponse` generator, NOT around the entire endpoint -- it replaces the generator argument passed to `StreamingResponse`.
  </action>
  <verify>
    ...
    Run `cd /Users/willmacdonald/Documents/Code/claude/second-brain/backend && grep -c 'foundryThreadId' src/second_brain/api/capture.py` -- should be >= 3 (follow-up read + initial capture write + wrapper logic).
  </verify>
  <done>
    `POST /api/capture/follow-up` endpoint exists and accepts `inbox_item_id`, `follow_up_text`, `follow_up_round`. `stream_follow_up_capture` generator passes `conversation_id` in ChatOptions. `InboxDocument` has `foundryThreadId` field. Initial capture stream captures `conversation_id` from Foundry updates. **Initial capture endpoints (`capture_text`, `capture_voice`) persist `foundryThreadId` to the inbox document via a wrapper generator after the stream completes when MISUNDERSTOOD is detected.** Orphan reconciliation logic copies classification to original and deletes the new doc.
  </done>
</task>
...
<task type="auto">
  ...
  <verify>
    ...
    Run `cd /Users/willmacdonald/Documents/Code/claude/second-brain/mobile && grep "api/capture/follow-up" lib/ag-ui-client.ts` -- should show the new URL.
    Run `cd /Users/willmacdonald/Documents/Code/claude/second-brain/mobile && grep "recategorize" app/conversation/\\[threadId\\].tsx` -- should show the PATCH call.
  </verify>
  <done>
...
3. Initial capture endpoints (`capture_text`, `capture_voice`) persist `foundryThreadId` to the inbox document in Cosmos after MISUNDERSTOOD is detected via `stream_with_thread_id_persistence` wrapper
4. `stream_follow_up_capture` exists and passes `conversation_id` in ChatOptions
5. `POST /api/capture/follow-up` endpoint reads `foundryThreadId` from Cosmos (400 if missing) -- this works because step 3 persists it
6. `POST /api/capture/follow-up` endpoint exists with orphan reconciliation
7. `handlePendingResolve` in inbox.tsx calls `handleRecategorize` (not `sendClarification`)
8. `sendFollowUp` URL is `/api/capture/follow-up`
9. Conversation screen uses PATCH for bucket selection (not SSE)
10. Existing recategorize PATCH endpoint is unchanged
</verification>
