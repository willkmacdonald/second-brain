...
<task type="auto">
  ...
  <action>
    ...
    2. **Update `classify_and_file`** status logic. Currently it sets `status = "classified" if confidence >= self._threshold else "low_confidence"`. Change to use a dual-threshold:
       ...
       **Also update the return string** for pending items. Currently classify_and_file always returns `f"Filed → {bucket} ({confidence:.2f})"`. Change to:
       - If status == "classified": return `f"Filed → {bucket} ({confidence:.2f})"` (unchanged)
       - If status == "pending": return `f"Filed (needs review) → {bucket} ({confidence:.2f})"`
       This distinct return string lets the capture screen show a "Captured (needs review)" toast for low-confidence items, signaling to the user that the item was filed but may need correction in the inbox.
    ...
    3. **Add `request_misunderstood` tool** as a new method on `ClassificationTools`. This tool:
       ...
       - Returns format: `"Misunderstood → {inbox_doc_id} | {question_text}"` (Unicode → to match adapter regex and other tool return formats)
    ...
    5. **Add tests for `request_misunderstood`**:
       ...
       - `test_request_misunderstood_returns_parseable_string`: Verify return matches `r"Misunderstood\s*→\s*([a-f0-9\-]+)\s*\|\s*(.+)"` (Unicode →) and contains a valid UUID.

    6. **Update `test_classify_and_file_low_confidence`**: Change the assertion from `status == "low_confidence"` to `status == "pending"` since low-confidence items now get "pending" status. Also assert the return string contains "(needs review)" -- e.g., `assert "(needs review)" in result`.
  </action>
  <verify>
