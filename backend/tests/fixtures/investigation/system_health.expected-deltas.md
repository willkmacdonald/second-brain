# Expected deltas: investigation/system_health

Captured: 2026-05-09 17:34 UTC
Trace ID: af9628ec-c19a-4a2d-8ee7-618c969619d5 (documentary only -- investigation endpoint does not read X-Trace-Id)
Thread ID out: thread_Q42dDcTGFPvFbKLHrEGUdRgM
...
- SSE field values for contract fields: tool_call.tool == "system_health", done.thread_id present
...
- Specific health metrics will differ between captures (request counts, latencies, error rates are time-dependent)

## Notes

Agent called system_health tool with a 24-hour window. Response included request counts, latency percentiles, and error breakdown. The specific numbers are non-deterministic and time-dependent -- only the SSE event shape and span structure are load-bearing.
